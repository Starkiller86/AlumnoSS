...
<html>
    <head>
		...
		<script type='text/javascript'> 
			function confirma() {
				jConfirm('Eliminar el usuario lo borrara permanentemente del sistema. \xBFEst\xE1 seguro de eliminar el usuario?', 'Atención', function(r) { if( r ) { document.forms["eliminar"].submit(); }})
				return false
			}
		</script>
    </head>
	<body onLoad="document.getElementById('name').focus()"> 
		<br><fieldset class="frame">
			<div id="legend">Eliminar usuarios</div>
		...
		<?php
			if(isset($_GET['usuario'])&&$_GET['usuario']!=0){
				...
				echo ("<div align = 'center'>
					<form method='post' id='eliminar' action='../php/deleteUser.php' name='baja' onSubmit='return confirma()'>
...
<?php
	...
	if(isset($_GET['a'])) {
		if($_GET['a']=="A0001") {
			echo "<script language='javascript' type='text/javascript'>jAlert('Se ha eliminado el usuario correctamente.', 'Atención')</script>";
		}
	}
?>
