<?php
	...
	if(isset($_GET['id'])) {
		...
		// 	$result = mysqli_query($query) or header("Location:error.php?emysql=".mysqli_error($conn));
		...
		// 		$result = mysqli_query($query) or header("Location:../forms/error.php?emysql=".mysqli_error($conn));
		// 	}
		// 	else{
		...
		// $res=mysqli_query($query, $conn) or header("Location:error.php?emysql=".mysqli_error($conn));
		...
		fclose($fp);
		header("Location:../forms/ConstancyDocForm.php?id=$_GET[id]&a=A0001");
	}
	else {
